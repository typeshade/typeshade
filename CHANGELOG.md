...
- **Two refusals around a vector of doubles name the reason and a remedy that compiles**
  ...
  sentence. Where a splat fixes nothing, the text is unchanged: `Types must match.` for a
  bitwise operator, an integer, another width, a vector of `f32`, and a `vecNf64` given to a
  declared `f64`. No code changes.
