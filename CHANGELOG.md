...
- **Every swizzle and a vector index draw no error in the editor or in `typeshade check`** (Rule
  12.7). The ambient library declared a vector's components and its prefix swizzles only, so
  `v.yx`, `v.xx`, `v.zyx`, `c.bgra` and `v.xyzz`, all ordinary WGSL the compiler takes, were
  TS2339 (the last with a wrong "Did you mean 'xyz'?"), and `v[i]` was TS7053, so
  `typeshade check` failed each such program. Each vector type is now an interface with every
  swizzle its size admits, typed by its length, and a native vector takes an index as WGSL does.
