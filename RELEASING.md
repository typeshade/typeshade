...
Do this once, before the first release. Both paths are supported; **trusted publishing is the
recommended one** because it stores no credential anywhere.
...
4. Save. Leave the repository's `NPM_TOKEN` secret **unset** — if it exists, the workflow uses
   it instead and trusted publishing is never attempted.
...
Use this if trusted publishing is unavailable or a release fails on the OIDC exchange.

1. npmjs.com → your avatar → **Access Tokens** → **Generate New Token** → **Granular Access
   Token**.
2. Scope it to the `typeshade` package only, permission **Read and write**, with the shortest
   expiry you are willing to renew. Do not create a classic "Automation" token scoped to every
   package you own.
3. In GitHub: **Settings → Secrets and variables → Actions → New repository secret**, named
   `NPM_TOKEN`.

The workflow writes it to `.npmrc` only when the secret is non-empty, and says in its log which
path it took. Remember to delete the secret if you later move to trusted publishing.
...
is a second placeholder held for a possible future split — **nothing is published to it**.
...
2. Move the `## Unreleased` entries in `CHANGELOG.md` under a new heading for the version, and
   ...
   is generated monorepo-era history — do not touch it.
...
Then create the release on GitHub — **Releases → Draft a new release** — choosing the tag you
just pushed, and publish it. ("Publish release" is the button that starts the workflow; saving
a draft does not.)
...
| Step                                       | What it is for                                                                                                                                                                                                 |
| ------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `verify`                                   | Calls `ci.yml` — type check, 224 test files, and the compile gate (every example emitted and compiled by Tint and a real WebGL2 context). Not a copy of CI; the same file.                                     |
...
| `ENEEDAUTH`, or a 401 on publish                                              | Neither auth path was configured. Either register the trusted publisher (§0) or set `NPM_TOKEN`. The workflow log says which path it attempted.                                                                     |
...
| The tarball check failed                                                      | An entry point does not resolve from the packed package. Nothing was uploaded. `src/publish-manifest.test.ts` D3 and D4 cover this case locally — run `bun run build && bun run test` and they should reproduce it. |
