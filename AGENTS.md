...
| `src/cli/`                 | The `typeshade` command (`bin` in `package.json`). `check.ts` merges the language service's diagnostics with `compile()`'s backend ones, `format.ts` prints them, `run.ts` is the command over an injected host, and `bin.ts` is the one file in `src/` that touches Node (dynamically, so the library build keeps `types: []`). Run it here with `bun src/cli/bin.ts check <paths>`.                                                                                                                                                                                                        |
