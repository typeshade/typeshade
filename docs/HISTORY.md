<!--
  MOVED HERE from CHANGELOG.md when TypeShade was separated from the X-GIS monorepo.
  The banner and the entries below are the file as its generator produced it; only this
  preface was added. See CHANGELOG.md for what has happened since the split.
-->

# History — the `@xgis/shader-dsl` era

TypeShade began inside the owner's earlier personal project, the **X-GIS monorepo**, as the
workspace `shader-dsl/`, published nowhere and imported as `@xgis/shader-dsl`. This file is the
changelog generated from that tree's git history, up to the point where the package was split
out into `typeshade/typeshade` and renamed `typeshade`.

It is kept verbatim because it is the only record of how the compiler got its present shape —
the breaking changes, the backend rewrites, the pass pipeline. Three things to know while
reading it:

- **Its links do not work.** Every entry links to a pull request in `github.com/X-GIS/X-GIS`,
  a repository that is not public and answers 403.
- **Its scope names are the monorepo's.** `**shader-dsl:**`, `**render:**` and `**map:**` are
  workspaces of that tree, not of this one.
- **It stops at the split.** Nothing after it is here; `CHANGELOG.md` at the repository root
  starts at that point and is maintained by hand.

`@xgis/shader-dsl` was never published to npm. The first published release of this code is
`typeshade@0.1.0`.
...
<!--
  ...
  What changed since this file was generated (run from a repo checkout):
    bun scripts/emit-changelog.ts --path shader-dsl --since deedb288508c
-->
