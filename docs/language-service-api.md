...
structurally as the `__@<tag>@<id>` property the checker reports, never by type name, or when
the operand is itself an operation on such a type, which TypeScript has typed as a `number` or a
`boolean` and whose shape the rules restore from its own operands. Arithmetic is not the only
such operation: a comparison of two vectors is a `boolean` to TypeScript and a mask to the
compiler, and the bitwise and shift operators, `**`, `~` and `!` lose the vector the same way.
The operators are one table, `ERASING_OPERATORS` in `projection.ts`, which the projection below
reads too.

| Code     | Why it fires                                                                                                                                                                                                                                                                                                                          | Dropped when                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| -------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `TS2362` | The left operand of an arithmetic operation is not `number` (`v * s`, `c.rgb * 0.5`, `v *= 2.`).                                                                                                                                                                                                                                      | The left operand's own type carries a vector or matrix brand, or the operand is an operation on one whose type TypeScript erased (`(a < b)` in `(a < b) * m`), whose shape the rules below restore. Per operand, not "either operand", so the string in `v * "x"` still reports through TS2363.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| `TS2363` | The right operand is not `number` (`m * v`, `2. * v`).                                                                                                                                                                                                                                                                                | The right operand is a vector or a matrix the same way.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| `TS2365` | The operator cannot be applied to the two types (`a + b` on two `vec3`).                                                                                                                                                                                                                                                              | The operator is one the front end gives its operands' own shape (arithmetic, a bitwise or shift operator, or a compound form of one; `ERASING_OPERATORS` in `projection.ts`), and either operand of the operation the code SPANS is a vector or a matrix. The spanned operation, not the innermost one starting at the same offset: a left-nested product starts where the operation it sits in starts, so asking the inner one made `n * 3. + v` report while `v + n * 3.` and `(n * 3.) + v` did not. A comparison draws TS2365 only when its operands really do not compare, a vector with a vector of another size or with a scalar, which is the compiler's `TS8003` as well, so it is not dropped here: the merge keeps the compiler's report of it.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| `TS2322` | An operation on a branded type is typed `number`, or `boolean` for a comparison, so the vector position it flows into (a return annotation, a local, a struct field, an assignment target, the return an arrow function's expression body stands for) looks unassignable.                                                             | The target type carries a vector or matrix brand AND the value either carries one too or applies an operator TypeScript types as a `number` or a `boolean` (`ERASING_OPERATORS`: arithmetic, the bitwise and shift operators, the comparisons, `-`, `+`, `~`, `!`) to a vector or matrix anywhere inside it (`normalize(a * 2.)` is already a `number` by the time the call is typed). `let n: f32 = v` keeps its TS2322: a scalar target is a `number` to TypeScript, so that mismatch is not the brand's doing.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| `TS2345` | The same `number` reaches a CALL: an argument position that wants a vector or matrix (`vec4(u.tint.rgb * k, u.tint.a)`, `normalize(v * s)`, `mix(a, b * 0.5, t)`, a user function's `f(v * s)`), or an inferred parameter a sibling argument already poisoned (`dot(a * 2., b)`, reported on the good `b`).                           | The argument applies such an operator to a vector or matrix anywhere inside it AND the SHAPE that operation would have produced (a `vec3 * f32` produces a `vec3`; `m * c` produces the vector, not the matrix; a comparison of two `vec3` the `vec3b` mask) is one the parameter at that index accepts. For an overloaded ambient constructor that matched no overload: when SOME overload accepts that shape there. Or: the parameter's type is one of the signature's own type parameters (`dot<T extends Numeric>`), the argument is branded, and another argument's arithmetic, of exactly this argument's shape, decided `T`. Either way every OTHER argument of the call has to fit its own position too, measured the same way, because TypeScript reports only the first argument that fails: a sibling in a position of the same type parameter has to have the shape that parameter settles on, and one in a position of a fixed type (`select`'s `cond`) the shape its own parameter declares. Arithmetic is required either way, so `vec4(1., c.a)`, `f(1.)` and `f(x)` with `x: f32` still report.                                                                                                                                |
...
| `TS2339` | A member read on a vector whose type an operation erased: `(n * 2.).xy`, `(a < b).x`, which TypeScript types `number` and `boolean`.                                                                                                                                                                                                  | The object's own TypeScript type is a `number`, a `boolean` or `any`, and the shape restored the way the rules above restore it is a vector. The front end checks every swizzle itself and names a bad one (`TS8022 .w out of range on vec3<f32>`), so a real mistake is reported once, by the compiler. A value TypeScript still types as a vector keeps its TS2339, and so does a matrix, which has no members.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| `TS2447` | `&`, `\|` or `^` on two masks, `(a < b) & (c < d)`: a comparison of vectors is a `boolean` to TypeScript, which refuses a bitwise operator on two booleans.                                                                                                                                                                           | Either operand is a vector to the compiler, as the rules above measure it. On `vecN<bool>` the three are WGSL's componentwise and, or and xor, and the `&&` and `\|\|` TypeScript suggests take a scalar `bool` only. On two scalar booleans the code stands.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
...
**A name declared from such an operation.** `const lit = albedo * d` declares `lit` as the
...
return, TS2339 at a swizzle. The program the service builds reads the document with the
compiler's type written into such a declaration (the projection, below), so TypeScript types
`lit` as the `vec3` it is, and the filters read no type from the front end. A declaration the
projection cannot type is one the compiler refused (`const t = a * b` with a `vec3` and a
`vec2`): the compiler reports the product and says nothing more about `t` (#171), and the
filters read `t` as the operation it was declared from, so `return t`, `t.x` and `f(t)` are
measured as `return a * b` would be, and draw nothing beside the compiler's one report. A name
TypeScript typed from anything else, a call, a swizzle or a literal, is judged by TypeScript's
own type.
...
| `TS8070` | `KERNEL_LOOP_ON_CPU`        | A WARNING, not an error: a kernel function's loop that the independence proof refused (Rule 8.22, change 0013), on the loop, naming the line and the author's names that stop it and the remedy; on the function when its body is not the kernel call's shape. The loop runs on the CPU, and the program is correct either way.                                                                                                                                                                                                |
...
the type TypeScript cannot infer: a `: vec2` after the name of every `const` or `let`, a local or
a module constant, that has no annotation, is not a `for` header's, has an initializer that
applies an operator TypeScript types as a `number` or a `boolean`, and has a front-end type that
is a vector (`f32`, `i32`, `u32`, `bool`, or an emulated double, `vec3f64`) or a matrix (`f32`,
or `f64` as `mat3x3<f64>`). The operators are one table, `ERASING_OPERATORS`: arithmetic, the
bitwise and shift operators, their compound forms, and the comparisons, which give two vectors
the `bool` vector of their width, with `-`, `+`, `~` and `!`; the diagnostics filters of §6 read
the same table for an operation used in place. The type comes from the front end's own record
of what it declared (`CompileTsSourceResult.symbols`), so it is the type the compiler uses. `host.ts` serves that text to TypeScript, and `service.ts` wraps every method:
...
A function that writes no return type returns what its body does (Rule 8.19), and when its
`return` applies one of those operators TypeScript types it a `number` or a `boolean` the same
way: `glow(uv).x` was TS2339,
`tint(glow(uv))` TS2345, and an arrow function handed to a call whose parameter returns a `vec2`
was TS2322 on its body, beside a global `Cannot find global type 'Promise'` that TypeScript's
elaboration of that error asks for under `lib: []`. So the projection also writes the return type
...
so `CompileTsSourceResult` keeps its shape. With the return type written in, TypeScript checks the
body against it, and the table's TS2322 rule above takes an arrow function's expression body
...
Plain `tsc` with `typeshade/shade` has no service in front of it and still types the local, or
the function, as a `number` (README, "What it covers, and what it does not"); the annotation
