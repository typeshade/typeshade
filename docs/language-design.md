...
A write to a static field through a class that does not declare it (`D.K = v`, `this.K = v`, `super.K = v`), `this.#x` where `this` is a class that extends the one declaring `#x`, `this` as a value, and a static block must be refused with the remedy; a static that fails only for a class that inherits it must be refused where that class calls it, and nowhere else.
