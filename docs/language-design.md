...
| Rule 12.7           | the ambient library is narrower than the compiler in two places: a WHOLE-binding write (`s = 1.` on a `storage<f32, "read_write">`) is `TS2588` in the editor, because a binding is `declare const` (Rule 6.1) and no value type can make a `const` assignable, while the compiler emits it; and a swizzle outside the components and the prefix swizzles (`v.yx`, `v.zyx`) is `TS2339`, where the compiler takes every pick of one set (`parseSwizzle`)                                                                                         | surface §49 records both; #210 declares every swizzle                                                        |
