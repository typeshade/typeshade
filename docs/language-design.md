...
- Enforced by: `TS8035 CLASS_MEMBER` for a static block, for `this` as a value, for `this.#x` through a class that extends the one declaring it (`"Derived" has no "#k": a private static is the class's own, and TypeScript throws where a body "Derived" inherits reaches it through "this". Name the class that declares it, "Base.#k".`) and for `super.K = v`; `TS8005 CONST_ASSIGN` for a write to a `readonly` static and for one through a class that does not declare it (`"Derived.count" is a static "Base" declares, and assigning it through "Derived" would give "Derived" a copy of its own in TypeScript. Write "Base.count".`); `TS8013 HOST_STMT` for `new this()` outside a static member; `TS8035 CLASS_MEMBER` for a static declared to return the class that declares it that builds its value with `new this()` where a class inherits it (`"Base.make" builds its value with "new this()", so "Derived.make()" returns a Derived, but it is declared to return a Base, which is the type the editor gives the call. Declare the class the call names: static make<C extends Base>(this: { new (): C; SCALE: f32 }): C`), pinned with the editor's half by `src/compiler/ts/class-syntax.test.ts`; pinned by `src/compiler/ts/class-syntax.test.ts` and `src/compiler/ts/class-methods.test.ts`, which also hold the three CPU paths to one value for a static a derived class inherits and for `super` in a static member; `examples/class-builder.shade.ts` in the compile gate.
...
- Enforced by: `toShader` and `fromShader` in `src/core/host-values.ts`, which the generated module calls at every argument and result; the view's types, from `hostTypeOf` in `src/compiler/ts/host-face.ts`; pinned by `src/compiler/ts/host-face.test.ts`, which checks every row in the view, a `Float32Array` vector, each refusal's text, and that a result aliases no argument; the binding rows by `src/compiler/ts/host-entry.test.ts` and `src/compiler/ts/host-draw.test.ts`.
...
- Derives from: change `0013` in `changes/` (roadmap item 15); #252; Rule 7.5; `docs/dx.md`.
- Enforced by: `proveKernels` in `src/core/passes/parallel-loop.ts`, whose facts `src/compiler/ts/kernel-loops.ts` words as `TS8070 KERNEL_LOOP_ON_CPU` in the author's names; pinned by `src/core/passes/parallel-loop.test.ts`, every accepted form and refusal #252 measured, and by `src/compiler/ts/kernel-loops.test.ts`, each refusal's text in the compiler and in the language service on the same source. The call's dispatch is `lowerKernel` in `src/core/passes/kernel-lower.ts`, which lowers a function whose every loop is an accepted map to one `@compute` entry per loop, run by `callKernel` in `src/core/host-kernel.ts` on WebGPU, and on the CPU tier where there is none or the function does not lower (a reduction, a refused loop, a `bool`, a module binding: later parts of change 0013); pinned by `src/compiler/ts/host-kernel.test.ts` on the CPU tier against the oracle, and by the import journey, which calls three kernel functions on WebGPU in Chromium.
...
- Enforced by: `TS8036 MATH_ARGUMENT` and `src/compiler/ts/lower/math-args.ts`, whose rule for a name with SUPPORTED `core.def` rows is derived from those rows (0017); `TS8019` for `f64(a, b)` (`f64() expects 1 argument.`) and `surface-names.test.ts` (`the f64 family declares only the constructor forms WGSL gives the type it stands in for`, `the compiler refuses a second argument to f64`).
...
**Rule 12.7.** The language service and the compiler must name one vocabulary: the ambient library's declarations are derived from the compiler's own tables and never retyped, a builtin's overloads from Tint's `core.def` through the TypeShade overlay, and a program the compiler accepts must draw no error in the editor or in `typeshade check`.
