...
| Rule 12.7           | the ambient library is narrower than the compiler in places: a WHOLE-binding write (`s = 1.` on a `storage<f32, "read_write">`) is `TS2588` in the editor, because a binding is `declare const` (Rule 6.1) and no value type can make a `const` assignable, while the compiler emits it; a SQUARE matrix column (`m[0] = vec4(1.)`) is `TS2322` on `never`, because the square aliases resolve the element through a conditional type (the non-square ones are clean)                                                                            | surface §49 records both                                                                                     |
