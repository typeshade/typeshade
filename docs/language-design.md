...
6. The storage-texture vocabulary and the type machinery are names TypeScript needs in order to refer to a shape; none is a shader value, and where a row does name a set of _values_ — a storage texture's formats and access words, a storage buffer's two access words in `StorageBufferAccess` — those values are WGSL enumerants passed as strings.
...
| Rule 12.7           | the ambient library is narrower than the compiler in places: `textureLoad` declares `vec2i` coordinates where WGSL and the compiler also take `vec2u`; a WHOLE-binding write (`s = 1.` on a `storage<f32, "read_write">`) is `TS2588` in the editor, because a binding is `declare const` (Rule 6.1) and no value type can make a `const` assignable, while the compiler emits it; a SQUARE matrix column (`m[0] = vec4(1.)`) is `TS2322` on `never`, because the square aliases resolve the element through a conditional type (the non-square ones are clean) | #157, #147; surface §49 records the last two                                                         |
