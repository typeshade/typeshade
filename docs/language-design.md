...
PR #166 is not on this branch, and this tree does not yet give the case the sentence Rule 2.3 asks for.
An `f64` passed, returned or assigned where an `f32` is declared is refused at the front end with `TS8003` (`Argument 1 of "g" type mismatch.`; `Function "f" return type mismatch: declared f32, got f64.`), and neither sentence names a remedy.
...
- Enforced by: the emit goldens (`examples/emit-goldens.test.ts`) for the spelling; the three exclusions are not enforced on this tree: `let $a: f32`, `let _: f32` and a parameter `__a` are emitted as written with zero diagnostics (Appendix B).
...
- Enforced by: not enforced on this tree; PR #165 adds `TS8068 RESERVED_NAME` (Appendix B).
...
- Enforced by: `sanitizeReservedIdents` in `src/core/backends/glsl-sanitize.ts` for locals and parameters (the rename); the module-surface warning and the fail-closed step are PR #165 (Appendix B).
...
**Rule 4.8.** `mat2` and `mat3` are declared in the ambient library and refused by the compiler until #149 lands; `mat4` and `mat4x4` are the one matrix shape.
...
**Rule 5.2.** A written number beside a typed peer in an arithmetic operator or a builtin call takes the peer's kind; a written number beside an `f64` vector becomes an `f64` literal carrying the full double.
...
- Enforced by: `retargetLit` in `src/compiler/ts/numeric.ts` (the lift is guarded on `isVec64(peer)`), `src/compiler/ts/int-lit-coerce.test.ts` and `int-lit-context.test.ts` (the `min(1, i)` case).

Beside a scalar `f64` the tree does less than the rule says: in an arithmetic operator the literal stays an `f32` and the program is refused (`TS8003 Type mismatch: cannot + f64 and f32. Types must match.`), and in a builtin call it is widened from its `f32` rounding, so `min(x, 0.1)` carries `(0.1, 0.0)` and not the full double.
`f64(0.1)` is the spelling that carries the full double beside a scalar today; the scalar case is a debt of Appendix B (#151).
...
**Rule 9.2.** A builtin's signature is WGSL's, checked at the call; the two recorded exceptions are `atan(y, x)` (WGSL's `atan2`) and the ECMAScript arity of the `Math` members.

- Rationale: an argument shape Tint refuses should be refused at the source line, with the splat or the cast to write.
- Derives from: [Numeric Built-in Functions](https://gpuweb.github.io/gpuweb/wgsl/#numeric-builtin-functions); surface §10 (#57).
- Enforced by: `TS8036 MATH_ARGUMENT` and `src/compiler/ts/lower/math-args.ts`.
...
| `mat2x2` in a uniform buffer: pad to two `vec4` columns with `reflect()` agreeing, or refuse with the remedy                                                    | #149                                                | `mat2` and `mat3` are refused (Rule 4.8); `reflect()` throws on a `mat2` under std140                                                                                                                                   | measured on a WebGL2 driver (std140 column stride 16 against WGSL's align 8), then the maintainer |
| An `f64` across a stage boundary (an entry parameter, a varying, a return): refuse with `f32(x)`, or bridge silently as a pair                                  | #151, after the maintainer's decision of 2026-09-21 | an `f64` in an I/O position passes the front end and is refused by the backend (`TS8015`, `SD0044`) with a hint naming `toF32`, which is not authorable; the pair bridge is internal and never spelled (Rule 2.3, §2.4) | the maintainer                                                                                    |
...
| `mat4`, `mat4x4`                                                                                                                                   | `mat4x4<f32>`                                                     | `mat4x4` is WGSL [Matrix Types](https://gpuweb.github.io/gpuweb/wgsl/#matrix-types); `mat4` is TypeShade (family 4)       |
| `mat2`, `mat3`                                                                                                                                     | `mat2x2<f32>`, `mat3x3<f32>` once #149 lands; refused today       | TypeShade (family 4)                                                                                                      |
...
Not spelled on this tree: `f16` and the `h` aliases (Rule 4.7), `mat2x3` and the other non-square shapes (#149), `ptr` (Rule 8.8), `texture_external`, the 1d and 3d storage textures (#162, scheduled by the roadmap).

## Appendix B: rules the current tree does not yet enforce

| Rule                           | What is not enforced                                                                                                                                                                                                                                      | Issue                                                                                                                |
| ------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| Rule 2.3, Rule 12.1, Rule 12.6 | an `f64` where an `f32` is declared is refused with a `TS8003` that names no remedy, and in an entry I/O position by the backend (`TS8015`/`SD0044`) with a hint naming the unauthorable `toF32`; the one-sentence refusal naming `f32(x)` does not exist | #151 (PR #166 is not on this branch)                                                                                 |
| Rule 3.2                       | `$` in a name, a name that is exactly `_`, and a `__` prefix are emitted as written (`var $a: f32`, `var __a: f32` reach Tint)                                                                                                                            | PR #165 is the nearest (`TS8068 RESERVED_NAME` is the same check on a different list); no issue names this shape yet |
...
| Rule 12.7                      | the ambient library is wider than the compiler in places (`mat2`, `mat3`) and narrower in others (unsigned texture coordinates)                                                                                                                           | #157, #147                                                                                                           |
