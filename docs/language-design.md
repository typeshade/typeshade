...
- Enforced by: `proveKernels` in `src/core/passes/parallel-loop.ts`, whose facts `src/compiler/ts/kernel-loops.ts` words as `TS8070 KERNEL_LOOP_ON_CPU` in the author's names; pinned by `src/core/passes/parallel-loop.test.ts`, every accepted form and refusal #252 measured, and by `src/compiler/ts/kernel-loops.test.ts`, each refusal's text in the compiler and in the language service on the same source. The call that dispatches an accepted loop is the next part of change 0013; until it lands a kernel function runs on the CPU oracle and is not callable through the import.
