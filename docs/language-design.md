...
A static field that holds a function, a function with type parameters, an `async` or generator function, and an expression body with no return type must be refused with the remedy.
...
- Rationale: `onHit = (d: f32): f32 => d * this.k` is how TypeScript code often writes a method, to keep `this` bound; a shader has no function value to hand anywhere, so the one thing such a field can be is the method it is written as, called as `o.onHit(d)`, and `this` is the object in both.
  A static one binds `this` to the class that declares it where a static method binds it to the class the call names (Rule 8.13), so it is written as the static method it means; type parameters need a generic function, and an expression body has nothing to infer its type from here, as a local function's has not (surface §4).
