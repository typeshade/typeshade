...
5. `mod`, `fill` and the five free spellings of a `Math` member WGSL has no builtin for (`log10`, `log1p`, `expm1`, `cbrt`, `hypot`) name operations WGSL has no function name for, and each of them reaches the emitted text as WGSL arithmetic or as a WGSL builtin under another name; the MEMBER keeps its ECMAScript source and the free name beside it does not, which is Rule 2.1(b) read at the site the name is declared (Rule 9.4). `random` is in the family for the same reason and is the one whose `Math` member is a DIFFERENT operation, which is why the member could never have accounted for it: ECMAScript's `Math.random()` is a seedless draw and does not compile here at all, while `random(seed)` hashes its seed (surface §55, #181, Appendix B). `discard` is WGSL's statement, declared as a `void` constant so that the bare identifier is a TypeScript statement (Rule 7.7). `sum`, `none` and `zip` are the array folds WGSL has no builtin for, each unrolled into WGSL arithmetic or into one call per element of the function the call hands over (Rule 8.18); the `any`, `all`, `min` and `max` folds are further signatures of WGSL names, and so are not rows.
...
| Rule 3.1            | `"use typeshade"` is accepted anywhere at top level, not only as the first statement: a file whose `class` declaration precedes the directive compiles with no diagnostic, and only a file with no directive at all is `TS8001`                                                                                                                                                                                                                                          | [#200](https://github.com/typeshade/typeshade/issues/200)                                                    |
...
| Rule 6.1            | `declare let x: uniform<T>` is accepted as a read-only uniform binding with no diagnostic, where surface §7 says a uniform must be `declare const`; the `TS8033` sentence for the form without `declare` names `declare let` as its remedy                                                                                                                                                                                                                               | surface §7, [#201](https://github.com/typeshade/typeshade/issues/201)                                        |
...
| Rule 7.3            | a non-empty `case` that ends without `break` is lowered without fall-through, silently diverging from TypeScript                                                                                                                                                                                                                                                                                                                                                         | [#202](https://github.com/typeshade/typeshade/issues/202) (#160's `case 0, 1` selectors landed, surface §52) |
...
| Rule 12.4           | a refused declaration (`declare const x: f32`, a `const` whose initializer is refused) leaves its name unbound, and every later use adds `TS8022 Unknown identifier` to the one refusal                                                                                                                                                                                                                                                                                  | #171                                                                                                         |
