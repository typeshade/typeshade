...
| Path tracing                      | Iterative bounces, a scene as data, random sampling, a stack for a BVH | `path-tracer` (#206); a BVH over a mesh needs a loop over a runtime length, which the loop rule refuses (#203)                                                                                       |
