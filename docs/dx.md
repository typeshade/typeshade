...
| ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------ |
| A program written the way a TypeScript developer writes it compiles, is clean in the editor, and runs on a GPU as written. A line spelled differently only to get past a gap names its issue                                  | `bun run gate:journeys` (CI `user-journeys`): the published tarball in a fresh project, each journey through `compile()`, the language service, plain `tsc`, WebGPU and the oracle | Holds on `main` for five journeys (#214)                           |
