...
| #   | Item                                                                                                                                                                                                                                                                           | Size | Issue | Notes                                                                                                                                                                                      |
| --- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ---- | ----- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 18  | `grad(f, param)` in forward mode as an IR pass: every `f32` and float vector becomes a value and a derivative, `if` and constant-bounded `for` differentiate through, the discontinuous builtins (`floor`, `step`, `sign`, `round`) have a zero derivative and the docs say so | M    |       | The result is an ordinary function that 15 can run on any tier. The pass and the host API `grad(m, fn, param)` are on this tree; a `"use typeshade"` spelling would need a §9.3 row first. |
