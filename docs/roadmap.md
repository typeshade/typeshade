...
| T19 | Accessors, private names `#x`, parameter properties, a field typed by its initializer, a static field that is written, `this` in a static member, `readonly` enforced                                           | M    |                                                           | **Shipped** (#190). Rules 8.11 to 8.14, surface §26. Each refusal names its fix; a private name is kept private by the front end, which does not run the checker.                                                                                        |
| T20 | `super` on an accessor and on a base method that writes its object, statics through a class that extends, `this` and `new this()` in a static member, `private` and `protected`, a chain of calls on one object | M    |                                                           | **Shipped** (#195). Rules 8.10, 8.11, 8.13 and 8.15, surface §26. A static a class inherits is lowered again for it, with `this` as that class; a chain that is a whole statement runs each call on the place it starts from.                            |
