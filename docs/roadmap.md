...
| 8   | Builtin breadth: `reflect`, `refract`, `faceForward`, `transpose`, `determinant`, `frexp`, `ldexp`, `modf`, `countOneBits`, `reverseBits`, `countLeadingZeros`, `countTrailingZeros`, `firstLeadingBit`, `firstTrailingBit`, `extractBits`, `insertBits`, the coarse and fine derivatives | S      |                                                                                                                                                                                                                                                                                             | One PR, one table, every entry in both docs tables and the oracle. `frexp` and `modf` are NOT in it: both return a struct, which `docs/use-typeshade-surface.md` §10 defers until the implicit result struct has a GLSL form. |
| 8a  | Builtin breadth 2: `quantizeToF16`, the pack and unpack spellings, `bitcast`, `any`/`all` on a bool, the zero-value constructors, inferred `array(...)`, typed `vecN<T>(...)`                                                                                                             | S      | [#150](https://github.com/typeshade/typeshade/issues/150)                                                                                                                                                                                                                                   | Every one is portable, and the pack ids already exist in the EDSL with no front-end route.                                                                                                                                    |
| 8b  | WGSL-only builtins behind a capability: `dot4U8Packed`, `dot4I8Packed`, `pack4xI8/U8`, `atomicCompareExchangeWeak`, `workgroupUniformLoad`, `textureBarrier`                                                                                                                              | M      | [#152](https://github.com/typeshade/typeshade/issues/152)                                                                                                                                                                                                                                   | GLSL ES 3.00 fails closed; the storage-atomics leg is [#138](https://github.com/typeshade/typeshade/issues/138).                                                                                                              |
...
| 9a  | Constructors and literals follow the WGSL table: `u32(-1)`, `f32(vec3)` refused, an integer literal first in an integer builtin, `abs(u32)` and integer `dot` on GLSL, the oracle's scalar `length`                                                                                       | S      | [#154](https://github.com/typeshade/typeshade/issues/154)                                                                                                                                                                                                                                   | `u32(-1)` emits `u32(-1.0)` today, and two PORTABLE rows emit GLSL overloads that do not exist.                                                                                                                               |
...
| T11 | Uniform layout, validated and padded: `array<T, N>` at stride 16 in a `var<uniform>`, `@size`/`@align`, `bool` and runtime arrays where WGSL forbids them, the emit and `reflect()` agreeing byte for byte         | M    | [#156](https://github.com/typeshade/typeshade/issues/156) | BLOCKER: a light array in a uniform is the first thing a renderer writes, and Tint refuses the emit while `reflect()` has already told the host to pack 16. |
| T12 | Entry IO attributes: `@interpolate(flat)` emitted on an integer varying, `@interpolate`, `@invariant` and `@blend_src` spelled, IO validation (numeric only, a duplicate location, compute IO, `@builtin` types)   | M    | [#158](https://github.com/typeshade/typeshade/issues/158) | BLOCKER for integer varyings; the GLSL writer already adds `flat`, so today the two targets disagree.                                                       |
| T13 | Operators and statements follow WGSL: a `u32` shift right-hand side, `~`, unary `+`, `-u32` refused, `case 0, 1`, `do…while`, a labelled break, `_ =`, calling an entry point, a whole-parameter write             | M    | [#160](https://github.com/typeshade/typeshade/issues/160) | BLOCKER for shifts; every row is a portable spelling.                                                                                                       |
| T14 | Derivative uniformity: refuse `textureSample*` and the derivatives in non-uniform control flow, or emit `diagnostic(off, derivative_uniformity)` on request                                                        | L    | [#161](https://github.com/typeshade/typeshade/issues/161) | BLOCKER; Tint's default diagnostic is an error, so the escape hatch is one line and the analysis is not.                                                    |
| T15 | Literal typing follows WGSL: an integer-written literal with no declared type is `i32`                                                                                                                             | M    | [#148](https://github.com/typeshade/typeshade/issues/148) | Breaking, so it needs the deprecation window item 25 defines.                                                                                               |
| T16 | Matrices: `mat2x2`, `mat3x3`, every `matCxR`, the constructors, `transpose`/`determinant` on every shape, `m * s`, `v * m`                                                                                         | L    | [#149](https://github.com/typeshade/typeshade/issues/149) | The f64 IR already carries `mat2f64T`/`mat3f64T`, so the std140 reason the refusal records no longer holds.                                                 |
| T17 | An author spelling for `enable`/`requires`, a `Capability` per WGSL extension name, `clip_distances`, `primitive_index`, `global_invocation_index`, `workgroup_index`, the `frag_depth` modes                      | M    | [#146](https://github.com/typeshade/typeshade/issues/146) | Five of the seven `enable` names have no capability today, and `clip_distances` is accepted with neither.                                                   |
| T18 | The f64 holes: the result type of `length`/`distance`/`dot` on a `vec64`, mixed operands, declared literals, components, narrowing constructors, author-time refusals that list the twins, a docs § and an example | M    | [#151](https://github.com/typeshade/typeshade/issues/151) | BLOCKER row: the reductions are typed `f32` at the front end while the pass emits the pair, so a correct program fails on its own return type.              |
...
| 13a | Texture argument and stage checks: an integer layer on a storage array, `f32` for `level`, `bias` and `depth_ref`, the storage coordinate's width, the 1d coordinate's kind, and the `textureStore` and `textureSample*` stage rules in one table | S    | [#145](https://github.com/typeshade/typeshade/issues/145) | Two of these are Tint-invalid emits today: a non-constant `i32` in an `f32` slot, and a storage read reachable from a vertex entry.                              |
| 13b | `textureDimensions(t, level)`, `textureNumLayers` on a storage array, a module `const` gather component, `u32` coordinates in the editor, the `bgra8unorm` and tier-1 formats, `read_write` formats behind a host feature                         | S    | [#147](https://github.com/typeshade/typeshade/issues/147) | The registry already spells the level query; the ambient library types the coordinate `i32` where the compiler and the spec take either.                         |
