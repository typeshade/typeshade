...
//
// WHAT AN EMPTY ALLOWLIST MEANS FOR THIS FILE. A2/A3/A4 police the rows, so with none left
// they are vacuously green and carry no information. A1 is the ratchet from here: it goes red
// the moment an undocumented export joins the surface, and its message points the author at
// the (still present, still empty) DEBT/UNDOCUMENTED tables as the honest way to land one.
//
 ...
 *
 *  EMPTY, and that is the finished state: the debt it was built to track is paid off. It stays
 *  because the next undocumented export needs somewhere to declare itself, and because A1's
 ...
 *
 *  NOW EMPTY — every public export across all four API subpaths carries a doc comment. With no
 *  rows left, arms A2/A3/A4 are vacuously green and stop carrying information; A1 is the whole
 ...
 *
 *  THE one place that question is answered, so the real surface and the synthetic probe in the
 ...
 *
 *  Exists for the sanity arm's "stuck at true" half, which can no longer be founded on the real
 *  surface now that nothing in it is undocumented. Compiled in memory — no fixture file on disk
...
function exportsOf(sub: string): readonly Def[] {
  ...
  return checker.getExportsOfModule(mod).map((sym) => {
    ...
    // Generating 167 of them would empty the debt list below in one commit and publish 167
    // pages that say nothing: every row green, the reference no better, and the gate silent
...
describe('#1695 — reader sanity (every arm below is vacuous without these)', () => {
  ...
  it('the doc reader distinguishes documented from undocumented', () => {
    ...
    //
    // The "stuck at false" half still reads the REAL surface: >100 documented exports is a
    // fact about this package that only a broken reader could miss.
    //
    // The "stuck at TRUE" half cannot, any more. It used to assert that at least one real
    // export read as undocumented — which was true only while debt existed, and the debt is
    // now paid off (175 → 0). Asserting it against the real surface today would fail for the
    // RIGHT reason and, worse, "fixing" it by deleting the arm would leave nothing proving the
    // reader can ever answer false: A1 would then pass over an empty set forever, and a reader
    // that silently began reporting everything as documented would look identical to a fully
    // documented package. That is §12's assertion-that-failed-either-way, arriving by success.
    //
    // So that half is re-founded on a SYNTHETIC pair instead, which discriminates regardless of
