...
describe('every divergent spelling is asserted as text, not only as divergent', () => {
  ...
  > = {
    ...
    ldexp: { wgsl: 'ldexp(a, b, c, d, e, f, g)', glsl: '(a * intBitsToFloat((b + 127) << 23))' },
    ...
    pack4x8unorm: {
      ...
      glsl: '(uint(round(clamp(a.x, 0.0, 1.0) * 255.0)) | (uint(round(clamp(a.y, 0.0, 1.0) * 255.0)) << 8) | (uint(round(clamp(a.z, 0.0, 1.0) * 255.0)) << 16) | (uint(round(clamp(a.w, 0.0, 1.0) * 255.0)) << 24))',
    },
