...
describe('wgslLayout — std140 / std430 offset engine', () => {
  ...
  it('vec3 has align 16 / size 12 (the classic std140 trap)', () => {
    const S = struct('S', [['a', f32T], ['b', vec3fT], ['c', f32T]])
  ...
  it('std140 rounds struct base alignment up to 16; std430 uses natural alignment', () => {
    const Inner = struct('Inner', [['x', f32T], ['y', f32T]])
...
describe('reflect — module metadata walker', () => {
  it('recovers bind groups, std140 uniform layout, and entry signatures', () => {
    const U = struct('Uniforms', [['mvp', mat4x4fT], ['viewport', vec4fT]])
    const m: ModuleDecl = {
      consts: [],
      structs: [U],
      ...
      funcs: [
        ...
        { name: 'cs', params: [], ret: { kind: 'void' }, attrs: ['@compute', '@workgroup_size(64)'], body: [] },
      ],
    }
    const r = reflect(m)
    expect(r.bindGroups).toEqual([{
      group: 0,
      entries: [{ group: 0, binding: 0, name: 'u', space: 'uniform', resourceKind: 'uniform-buffer', structName: 'Uniforms' }],
    }])
