...
const color: Expr = { op: 'construct', type: vec4fT, args: [featAtFid, lit(0, f32T), lit(0, f32T), lit(1, f32T)] }
    ...
    const scatter: ModuleDecl = {
      ...
      funcs: [{
        ...
        body: [
          ...
          { s: 'assign', target: index(varref('out_color', arrU32), lit(0, u32T), u32T), expr: packed },
        ],
      }],
    }
    expect(() => emitGlslModule(scatter, 'fragment', { emulateCompute: true })).toThrow(UnsupportedFeatureError)
  })
})
