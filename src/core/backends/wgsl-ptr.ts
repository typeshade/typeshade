...
export const ptrSpaceOf = (p: FuncDecl['params'][number]): PtrSpace => SPACES.get(p) ?? 'function';
...
export function pointerSpaces(m: ModuleDecl): ModuleDecl {
  ...
  const argSpaces = (
    call: Expr & { op: 'call' },
    inside: ReadonlyMap<string, PtrSpace>,
  ): PtrSpace[] => {
  ...
  for (const f of m.funcs) {
    ...
    const keys = [
      ...(wanted.get(f.name) ??
        new Set([keyOf(f.params.filter((p) => p.mode === 'inout').map(() => 'function'))])),
    ];
    keys.sort();
  ...
  return {
    ...m,
    funcs: funcs.map((f) => retarget(f, hasPointer(f) ? bindingOfDecl(f) : new Map())),
  };
}
