...
export function ifExpr<K extends string>(
  ...
  b.if(cond, () => { const val = thenVal(); vt ??= val.type; (iv.ref(val.type) as Node<K>).assign(val) })
    .else(() => { const val = elseVal(); vt ??= val.type; (iv.ref(val.type) as Node<K>).assign(val) })
  iv.commit(vt!)
...
export function condExpr<K extends string>(
  ...
  const iv = b.inferredVar()
  let vt: ShaderType | undefined
  const arm = (v: () => Node<K>) => () => { const val = v(); vt ??= val.type; (iv.ref(val.type) as Node<K>).assign(val) }
