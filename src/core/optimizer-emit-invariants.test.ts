...
import { fn, module, f32, vec4, vec2, f32T, vec4fT, u32T, u32, toF32, Var, Loop, matchExpr, structT, arrayT } from './ir'
...
describe('#763 P — optimizer/emit invariants', () => {
  it('P1: constFold + algebraicSimplify skip raw-Stmt fns (verbatim charter)', () => {
    ...
    const folded = constFold(twin).funcs[0]!.body.find((s) => s.s === 'return') as { expr: { op: string } }
  ...
  it('P2: licm seeds its temp counter past existing _licmN', () => {
    ...
    const primed: FuncDecl = {
      ...decl.decl,
      body: [{ s: 'let', name: '_licm0', expr: { op: 'lit', type: f32T, value: 1 } } as unknown as Stmt, ...decl.decl.body],
    }
    const out = licm(module({ funcs: [primed] }))
    const names = out.funcs[0]!.body.filter((s): s is Stmt & { s: 'let'; name: string } => s.s === 'let').map((s) => s.name)
  ...
  it('P3: matchExpr in a for header throws at lower time (the promised guard)', () => {
    const decl = fn('p3', { k: u32T }, ({ k }) => {
      const acc = Var(f32(0))
      Loop(u32(0), (i) => toF32(i).lt(matchExpr(k, [[0, f32(4)]], f32(8))), () => {
  ...
  it('P4: @interpolate(flat) float varyings emit GLSL `flat` on BOTH sides', () => {
    ...
    const vs = fn('vs', { vi: builtin('vertex_index', u32T) }, () =>
      VsOut.construct({ pos: vec4(0, 0, 0, 1), w: f32(1) }), { stage: 'vertex' })
    const fs = fn('fs', { in: VsOut }, (p) => vec4(p.in.w, 0, 0, 1), { stage: 'fragment', retAttr: '@location(0)' })
  ...
  it('P6: a reserved-word fn name is renamed at decl + call site; a reserved binding name fails loud', () => {
    ...
    const entry = fn('p6', { vi: builtin('vertex_index', u32T) }, ({ vi }) => {
    ...
    const glsl = emitGlslModule({ ...m, structs: [{ name: 'P6Out', fields: [{ name: 'pos', type: vec4fT, attr: '@builtin(position)', builtin: 'position' }] }] }, 'vertex')
  ...
  it('P7: mat2 in a std140 layout is rejected (WGSL/GLSL rules disagree)', () => {
    ...
    expect(() => wgslLayout({ name: 'M2', fields: [{ name: 'm', type: mat2T }] }, 'std140')).toThrow(/mat2/)
    // std430 (storage) keeps working — the divergence is uniform-only.
    expect(() => wgslLayout({ name: 'M2s', fields: [{ name: 'm', type: mat2T }] }, 'std430')).not.toThrow()
  })

  it('P8: a bare non-struct VERTEX output fails closed on GLSL (name-linked varyings)', () => {
    const vs = fn('p8', { vi: builtin('vertex_index', u32T) }, ({ vi }) => vec2(toF32(vi), 0), { stage: 'vertex', retAttr: '@location(0)' })
