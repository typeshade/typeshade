...
describe('optimize — algebraic simplification', () => {
  it('x + 0 -> x', () => {
    const m = module({ funcs: [fn('k', { x: f32T }, f32T, ({ x }, b) => { b.ret(x.add(0)) })] })
    const wgsl = emitModule(algebraicSimplify(m))
    expect(wgsl).not.toMatch(/\+\s*0\.0/)
  })

  it('x * 1 -> x', () => {
    const m = module({ funcs: [fn('k', { x: f32T }, f32T, ({ x }, b) => { b.ret(x.mul(1)) })] })
    expect(emitModule(algebraicSimplify(m))).not.toMatch(/\*\s*1\.0/)
  })

  it('x / 1 -> x', () => {
    const m = module({ funcs: [fn('k', { x: f32T }, f32T, ({ x }, b) => { b.ret(x.div(1)) })] })
    expect(emitModule(algebraicSimplify(m))).not.toMatch(/\/\s*1\.0/)
  })

  it('does NOT apply x * 0 -> 0 by default (not IEEE-sound)', () => {
    const m = module({ funcs: [fn('k', { x: f32T }, f32T, ({ x }, b) => { b.ret(x.mul(0)) })] })
    expect(emitModule(algebraicSimplify(m))).toMatch(/\*\s*0\.0/)
  })

  it('preserves oracle value-equality', () => {
    const m = module({ funcs: [fn('k', { x: f32T }, f32T, ({ x }, b) => { b.ret(x.add(0).mul(1)) })] })
