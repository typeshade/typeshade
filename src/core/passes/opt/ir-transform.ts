...
export function mapExpr(e: Expr, f: (e: Expr) => Expr): Expr {
  ...
  switch (e.op) {
    ...
    case 'varref':
      r = e; break
    case 'binop':
      r = { ...e, a: mapExpr(e.a, f), b: mapExpr(e.b, f) }; break
    case 'compare':
      r = { ...e, a: mapExpr(e.a, f), b: mapExpr(e.b, f) }; break
    case 'logical':
      r = { ...e, a: mapExpr(e.a, f), b: mapExpr(e.b, f) }; break
    case 'unop':
      r = { ...e, a: mapExpr(e.a, f) }; break
    case 'call':
      r = { ...e, args: e.args.map((a) => mapExpr(a, f)) }; break
    case 'construct':
      r = { ...e, args: e.args.map((a) => mapExpr(a, f)) }; break
    case 'member':
      r = { ...e, base: mapExpr(e.base, f) }; break
    case 'index':
      r = { ...e, base: mapExpr(e.base, f), idx: mapExpr(e.idx, f) }; break
    case 'select':
      r = { ...e, cond: mapExpr(e.cond, f), ifTrue: mapExpr(e.ifTrue, f), ifFalse: mapExpr(e.ifFalse, f) }; break
    case 'matchExpr':
      r = {
        ...e,
...
export function mapStmt(s: Stmt, f: (e: Expr) => Expr): Stmt {
  switch (s.s) {
    case 'let': return { ...s, expr: mapExpr(s.expr, f) }
    case 'var': return s.init !== undefined ? { ...s, init: mapExpr(s.init, f) } : s
    case 'assign': return { ...s, target: mapExpr(s.target, f), expr: mapExpr(s.expr, f) }
    case 'assignOp': return { ...s, target: mapExpr(s.target, f), expr: mapExpr(s.expr, f) }
    case 'return': return s.expr !== undefined ? { ...s, expr: mapExpr(s.expr, f) } : s
    case 'if': return {
      ...s,
      arms: s.arms.map((arm) => ({ cond: mapExpr(arm.cond, f), body: arm.body.map((b) => mapStmt(b, f)) })),
