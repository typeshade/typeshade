...
describe('composeModule', () => {
  it('swaps a top-level placeholder with the swap Stmt list', () => {
    const base = module({ funcs: [fn('f', {}, f32T, (_p, b) => { b.placeholder('ret') })] })
  ...
  it('descends into nested bodies (placeholder inside an If)', () => {
    const base = module({
      funcs: [fn('f', { x: f32T }, f32T, ({ x }, b) => {
        b.if(x.gt(0), (bb) => { bb.placeholder('hot') })
        b.ret(f32(0))
  ...
  it('throws on an un-swapped placeholder by default; allowUnswapped lets it survive', () => {
    const base = module({ funcs: [fn('f', {}, f32T, (_p, b) => { b.placeholder('ret'); b.ret(f32(0)) }, { allowEarlyReturn: true })] })
