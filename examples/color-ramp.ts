...
import {
  ...
  sin, fract, smoothstep, min, clamp, fwidth, mix, f32,
  location, builtin,
...
const ramp = fn('ramp', { x: f32T }, ({ x }) => {
  ...
  const c1 = vec3(0.99, 0.80, 0.45)
  const c2 = vec3(0.96, 0.50, 0.24)
...
const fs = fn('fs', { vo: VsOut.type }, ({ vo }) => {
  ...
  const val = clamp(
    sin(uv.x.mul(6.2832).add(t.mul(0.5))).mul(0.3).add(0.5)
...
export const colorRamp: ShaderExample = {
  id: 'color-ramp',
  title: 'Colour ramp',
  blurb: 'The data-driven choropleth ramp: a reusable ramp() function maps a value field through a 5-stop palette, with anti-aliased contour isolines. Set how many bands to draw.',
