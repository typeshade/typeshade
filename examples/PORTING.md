...
Since this table was written, **A1** (#19) and, in #8 A6, **A6-deriv**, **A6-discard** and
**A6-f64** (the cast) have landed. The fp64 family is still held by **N1** and **N2**, which
no issue item covers, so its rows have not moved.
...
| `a * b` (both vec3)                                                                                             | ✓                                                                                    |
| `f64(p.x)`, `bool(i)`                                                                                           | ✓ since #8 A6 — `SCALAR_CAST` now holds f32/i32/u32/bool/f64                         |
| `u.cx * u.cx` (f64 uniform field)                                                                               | ✓ — f64 **arithmetic** works; only the cast and the literal are missing              |
| `u.cx * 2.`                                                                                                     | ✗ `cannot * f64 and f32` — **N2**; `u.cx * f64(2.)` compiles since #8 A6             |
...
| `exp2(x)` / `saturate(x)` / `select(f,t,c)` / `fma(a,b,c)` / `atan(y,x)` / `a ** b`                             | ✓ since #8 A6                                                                        |
...
| `discard`                                                                                                       | ✓ since #8 A6, in a fragment entry and in a helper (vertex/compute entry rejects)    |
