...
fn fs_ship(vo: VsOut) -> @location(0) vec4<f32> {
  ...
  if ((_cse0 || (u.fp64 < 0.5))) {
    ...
    let cx_1 = df64_add(_cse1, vec2<f32>(dx, 0.0));
    let cy_1 = df64_add(_cse2, vec2<f32>(dy, 0.0));
    ...
    for (var j_1: u32 = 0u; (j_1 < 128u); j_1 = (j_1 + 1u)) {
      if (df64_le(df64_add(df64_mul(zx_1, zx_1), df64_mul(zy_1, zy_1)), _licm0)) {
        let nzx_1 = df64_add(df64_sub(df64_mul(zx_1, zx_1), df64_mul(zy_1, zy_1)), cx_1);
        zy_1 = df64_add(df64_mul(df64_abs(df64_mul(zx_1, zy_1)), _licm1), cy_1);
        zx_1 = nzx_1;
        it = (it + 1.0);
      }
    }
