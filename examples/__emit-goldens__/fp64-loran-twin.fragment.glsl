...
void main() {
  ...
  vec2 _lc0 = df64_add(_cse1, vec2(dx, 0.0));
  vec2 _lc1 = df64_add(_cse2, vec2(dy, 0.0));
  ...
  vec2 _gv0 = df64_add(vec2(pos.hi.x, pos.lo.x), _cse8);
  vec2 _lc2 = df64_sub(_gv0, _cse3);
  vec2 _gv1 = df64_add(vec2(pos.hi.y, pos.lo.y), _cse8);
  vec2 _lc3 = df64_sub(_gv1, _cse4);
  vec2 d1 = df64_sqrt(df64_add(df64_mul(_lc2, _lc2), df64_mul(_lc3, _lc3)));
  ...
  vec2 d2 = df64_sqrt(df64_add(df64_mul(_lc4, _lc4), df64_mul(_lc5, _lc5)));
  float th64 = df64_narrow(df64_fract(df64_mul(df64_sub(df64_add(d1, _cse8), df64_add(d2, _cse8)), vec2(0.25, 0.0))));
  float te64 = df64_narrow(df64_fract(df64_mul(df64_add(d1, d2), vec2(0.0625, 0.0))));
