...
void main() {
  ...
  vec2 px = df64_add(_cse1, vec2(dx, 0.0), _fp64_g);
  vec2 _cse2 = vec2(u.center.hi.y, u.center.lo.y);
  vec2 py = df64_add(_cse2, vec2(dy, 0.0), _fp64_g);
  float par64 = df64_narrow(df64_fract(df64_mul(df64_add(df64_floor(px, _fp64_g), df64_floor(py, _fp64_g), _fp64_g), vec2(0.5, 0.0), _fp64_g), _fp64_g));
  float _cse4 = uintBitsToFloat(floatBitsToUint(0.0));
  vec2 _cse3 = vec2(_cse4, _cse4);
  float fx64 = df64_narrow(df64_fract(df64_add(px, _cse3, _fp64_g), _fp64_g));
  float fy64 = df64_narrow(df64_fract(df64_add(py, _cse3, _fp64_g), _fp64_g));
