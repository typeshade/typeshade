...
fn fs_ship(vo: VsOut) -> @location(0) vec4<f32> {
  ...
  if ((_cse0 || (u.fp64 < 0.5))) {
    ...
    let _v13 = df64_add(_cse1, vec2<f32>(_v3, 0.0), _fp64_g);
    let _v14 = df64_add(_cse2, vec2<f32>(_v4, 0.0), _fp64_g);
  ...
  let _gv0 = (_v21 * _v21);
  let _v22 = (_gv0 * (3.0 - (_v21 * 2.0)));
  return vec4<f32>((mix(mix(vec3<f32>(0.06, 0.02, 0.05), vec3<f32>(0.95, 0.45, 0.08), _v22), vec3<f32>(1.0, 0.93, 0.75), _gv0) * (1.0 - _v20)), 1.0);
}
