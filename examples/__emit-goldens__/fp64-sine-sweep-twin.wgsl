...
fn fs_sweep(vo: VsOut) -> @location(0) vec4<f32> {
  ...
  let _gv0 = (sx * 25.132741228718345);
  let arg64 = df64_add(u.base, vec2<f32>(_gv0, 0.0), _fp64_g);
  let _cse1 = bitcast<f32>(bitcast<u32>(0.0));
  let y64 = df64_narrow(df64_sin(df64_add(arg64, vec2<f32>(_cse1, _cse1), _fp64_g), _fp64_g));
  let y32 = sin((df64_narrow(u.base) + _gv0));
  let v = select(y64, y32, isF32);
  let py = ((vo.uv.y - 0.5) * 2.0);
  let px = (2.0 / u.resolution.y);
  let gxf = fract((sx * 10.0));
  let gyf = fract(((py + 1.0) * 4.0));
  let dgx = min(gxf, (1.0 - gxf));
  let dgy = min(gyf, (1.0 - gyf));
  let aaCx = (30.0 / u.resolution.x);
  let aaCy = (20.0 / u.resolution.y);
  ...
  let fill = step(py, v);
  let rgb1 = mix(rgb0, vec3<f32>(0.62, 0.74, 0.9), (fill * 0.4));
  let ink = (1.0 - smoothstep((px * 1.2), (px * 3.0), abs((v - py))));
  let rgb2 = mix(rgb1, vec3<f32>(0.13, 0.16, 0.3), (ink * 0.85));
  let _lc0 = (px * 1.5);
  let axis = min(smoothstep(0.0, _lc0, abs(py)), smoothstep(0.0, _lc0, (abs((sx - 0.5)) * 2.0)));
  let rgb = mix(vec3<f32>(0.35, 0.33, 0.3), rgb2, axis);
