...
void main() {
  ...
  vec2 _gv0 = vec2(_v0, 0.0);
  vec2 _v8 = df64_mul(df64_add(_cse1, vec2(_v5, 0.0), _fp64_g), _gv0, _fp64_g);
  vec2 _cse2 = vec2(u.center.hi.y, u.center.lo.y);
  vec2 _v9 = df64_mul(df64_add(_cse2, vec2(_v6, 0.0), _fp64_g), _gv0, _fp64_g);
  vec2 _gv1 = df64_floor(_v8, _fp64_g);
  float _v10 = df64_narrow(_gv1);
  vec2 _gv2 = df64_floor(_v9, _fp64_g);
  float _v11 = df64_narrow(_gv2);
  float _cse4 = uintBitsToFloat(floatBitsToUint(0.0));
  vec2 _cse3 = vec2(_cse4, _cse4);
  float _v12 = df64_narrow(df64_fract(df64_add(_v8, _cse3, _fp64_g), _fp64_g));
  float _v13 = df64_narrow(df64_fract(df64_add(_v9, _cse3, _fp64_g), _fp64_g));
  float _v14 = (df64_narrow(df64_fract(df64_mul(df64_add(_gv1, _gv2, _fp64_g), vec2(0.5, 0.0), _fp64_g), _fp64_g)) * 2.0);
