...
vec4 fs_tiles_impl(VsOut vo) {
  ...
  vec2 _v8 = df64_mul(df64_add(_cse1, vec2(_v5, 0.0)), _gv0);
  vec2 _cse2 = vec2(u.center.hi.y, u.center.lo.y);
  vec2 _v9 = df64_mul(df64_add(_cse2, vec2(_v6, 0.0)), _gv0);
  vec2 _gv1 = df64_floor(_v8);
  float _v10 = df64_narrow(_gv1);
  vec2 _gv2 = df64_floor(_v9);
  ...
  float _v14 = (df64_narrow(df64_fract(df64_mul(df64_add(_gv1, _gv2), vec2(0.5, 0.0)))) * 2.0);
