...
fn fs_rtc(vo: VsOut) -> @location(0) vec4<f32> {
  ...
  let ex64 = df64_narrow(df64_sub(df64_add(_cse1, vec2<f32>(dx, 0.0)), df64_add(_cse2, _cse3)));
  let _cse4 = vec2<f32>(u.center.hi.y, u.center.lo.y);
  let _cse5 = vec2<f32>(u.mark.hi.y, u.mark.lo.y);
  let ey64 = df64_narrow(df64_sub(df64_add(_cse4, vec2<f32>(dy, 0.0)), df64_add(_cse5, _cse3)));
