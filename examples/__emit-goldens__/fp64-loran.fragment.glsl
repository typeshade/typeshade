...
vec2 df64_mul(vec2 a, vec2 b) {
  ...
  vec2 _v0 = vec2((a.x * _cse0), (a.y * _cse0));
  vec2 _v1 = vec2((b.x * _cse0), (b.y * _cse0));
  vec2 _v2 = df64_twoProd(_v0.x, _v1.x);
...
vec4 fs_loran_impl(VsOut vo) {
  vec2 _cse13 = vec2(12000000.0, 0.0);
  vec2 _cse14 = vec2(34000000.0, 0.0);
  vec2 _cse15 = vec2(19000000.0, 0.0);
  vec2 _cse16 = vec2(26000000.0, 0.0);
  DF64Vec2 _cse11 = DF64Vec2(vec2(_cse13.x, _cse14.x), vec2(_cse13.y, _cse14.y));
  DF64Vec2 _cse12 = DF64Vec2(vec2(_cse15.x, _cse16.x), vec2(_cse15.y, _cse16.y));
  vec2 _cse7 = _cse11.hi;
  vec2 _cse8 = _cse11.lo;
  vec2 _cse9 = _cse12.hi;
  vec2 _cse10 = _cse12.lo;
  bool _cse0 = (vo.uv.x < 0.5);
  vec2 _cse1 = vec2(u.center.hi.x, u.center.lo.x);
  vec2 _cse2 = vec2(u.center.hi.y, u.center.lo.y);
  vec2 _cse3 = vec2(_cse7.x, _cse8.x);
  vec2 _cse4 = vec2(_cse7.y, _cse8.y);
  vec2 _cse5 = vec2(_cse9.x, _cse10.x);
  vec2 _cse6 = vec2(_cse9.y, _cse10.y);
  ...
  float _v9 = df64_narrow(df64_fract(df64_mul(df64_sub(_v7, _v8), vec2(0.25, 0.0))));
