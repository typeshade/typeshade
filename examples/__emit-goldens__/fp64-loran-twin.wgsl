...
fn fs_loran(vo: VsOut) -> @location(0) vec4<f32> {
  ...
  let _lc0 = df64_add(_cse1, vec2<f32>(dx, 0.0));
  let _lc1 = df64_add(_cse2, vec2<f32>(dy, 0.0));
  ...
  let _gv0 = df64_add(vec2<f32>(pos.hi.x, pos.lo.x), _cse8);
  let _lc2 = df64_sub(_gv0, _cse3);
  let _gv1 = df64_add(vec2<f32>(pos.hi.y, pos.lo.y), _cse8);
  let _lc3 = df64_sub(_gv1, _cse4);
  let d1 = df64_sqrt(df64_add(df64_mul(_lc2, _lc2), df64_mul(_lc3, _lc3)));
  ...
  let _lc4 = df64_sub(_gv0, _cse5);
  let _lc5 = df64_sub(_gv1, _cse6);
  let d2 = df64_sqrt(df64_add(df64_mul(_lc4, _lc4), df64_mul(_lc5, _lc5)));
  let th64 = df64_narrow(df64_fract(df64_mul(df64_sub(df64_add(d1, _cse8), df64_add(d2, _cse8)), vec2<f32>(0.25, 0.0))));
  let te64 = df64_narrow(df64_fract(df64_mul(df64_add(d1, d2), vec2<f32>(0.0625, 0.0))));
  let pos32 = vec2<f32>((df64_narrow(_cse1) + dx), (df64_narrow(_cse2) + dy));
  let d1f = length((pos32 - vec2<f32>(df64_narrow(_cse7), df64_narrow(_cse9))));
  let d2f = length((pos32 - vec2<f32>(df64_narrow(_cse10), df64_narrow(_cse11))));
