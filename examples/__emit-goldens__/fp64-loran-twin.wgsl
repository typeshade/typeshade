...
fn fs_loran(vo: VsOut) -> @location(0) vec4<f32> {
  ...
  let _gv0 = df64_add(vec2<f32>(pos.hi.x, pos.lo.x), _cse8, _fp64_g);
  let _lc2 = df64_sub(_gv0, _cse3, _fp64_g);
  let _gv1 = df64_add(vec2<f32>(pos.hi.y, pos.lo.y), _cse8, _fp64_g);
  let _lc3 = df64_sub(_gv1, _cse4, _fp64_g);
  let d1 = df64_sqrt(df64_add(df64_mul(_lc2, _lc2, _fp64_g), df64_mul(_lc3, _lc3, _fp64_g), _fp64_g), _fp64_g);
  ...
  let d2 = df64_sqrt(df64_add(df64_mul(_lc4, _lc4, _fp64_g), df64_mul(_lc5, _lc5, _fp64_g), _fp64_g), _fp64_g);
  let th64 = df64_narrow(df64_fract(df64_mul(df64_sub(df64_add(d1, _cse8, _fp64_g), df64_add(d2, _cse8, _fp64_g), _fp64_g), vec2<f32>(0.25, 0.0), _fp64_g), _fp64_g));
  let te64 = df64_narrow(df64_fract(df64_mul(df64_add(d1, d2, _fp64_g), vec2<f32>(0.0625, 0.0), _fp64_g), _fp64_g));
