...
5. a browser bundle of `src/gpu.ts` calls two `@compute` entries through the import in a page with WebGPU (change 0016), one of them through a barrier, which has no CPU tier, and both match the reference; it then draws two full-screen fragment entries of `src/draw.shade.ts` on WebGPU, WebGL2 and the CPU tier, each frame against the reference, and a sampled texture's draw on the CPU is refused with the reason. It calls the particles journey's `step` for 20 frames and draws the plasma journey's `fs` through the import, each against that journey's own reference, and checks that a production build records no `console.*` call while `vite dev` prints an entry's calls from WebGPU in order. It also calls three kernel functions of `src/loops.shade.ts` (change 0013), whose loops run on WebGPU one invocation per iteration, each against the f32 reference, and one with an array too short for its loop, which is refused before anything is uploaded.
